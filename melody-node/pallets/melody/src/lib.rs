...
use sp_std::prelude::*;
use melody_primitives::nft::{NftInfo, AccountIdOrCollectionNftTuple};
use melody_primitives::types::{CollectionId, NftId};
...
pub mod pallet {
		...
		where T: pallet_uniques::Config<CollectionId = CollectionId, ItemId = NftId>,
	{
		...
		pub fn list(
			...
			let nft = pallet_music::Pallet::<T>::nfts(collection_id, nft_id)
				.ok_or(Error::<T>::TokenDoesNotExist)?;

			pallet_music::Pallet::<T>::check_is_transferable(&nft)?;
			pallet_music::Pallet::<T>::set_lock((collection_id, nft_id), true);
		...
		pub fn unlist(
			...
			pallet_music::Pallet::<T>::set_lock((collection_id, nft_id), false);
			// Remove from storage
			ListedNfts::<T>::remove(collection_id, nft_id);
	...
	where T: pallet_uniques::Config<CollectionId = CollectionId, ItemId = NftId>,
{
	fn do_buy(
		...
		pallet_music::Pallet::<T>::send(owner_origin, collection_id, nft_id, new_owner)?;
	...
	fn is_nft_owned_by_nft(collection_id: CollectionId, nft_id: NftId) -> bool {
		...
		if let Some(current_owner) = owner {
			let current_owner_cid_nid =
				pallet_music::Pallet::<T>::decode_nft_account_id::<T::AccountId>(current_owner);
